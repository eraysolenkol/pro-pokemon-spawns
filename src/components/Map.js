    ...
    useEffect(() => {
        ...
        pokemons.forEach((pokemon) => {
            for (let i = 0; i < pokemon.spawns.length; i++) {
                if (pokemon.spawns[i].map && pokemon.spawns[i].map.toLowerCase() === params.mapName.toLowerCase().replace("-"," ")) {
                    matches.push({ pokemon: pokemon.name, id:pokemon.pokedexID, spawn: pokemon.spawns[i] });
                }
            }
        });
        setSpawns(matches);
            ...
            return (
                <>
                    ...
                    <div className='container'>
                        <p>{params.mapName.replace("-"," ").toUpperCase()}</p>
